...
public class Payment implements Task {
    ...
    public <T extends Actor> void performAs(T actor) {
        actor.attemptsTo(
                ...
                Click.on(By.xpath("//android.widget.TextView[@text='"+this.country+"']")),
                Click.on(PaymentPageElements.PAYMENT_BUTTON),
                Click.on(PaymentPageElements.CONFIRM_BUTTON)
        );
    }
    public static  Payment type(String phone, String name, String amount,String country){
        return  instrumented(Payment.class,phone,name,amount,country);
    }
}